...
Parallel CPU version with optimized thread management for high-core systems
"""
...
# Limit TensorFlow threading in main process
...
CONFIG = {
    ...
    'num_frames': 20,
    'frame_size': (224, 224),
    'num_workers': 64,  # Reduced from 192 to prevent thread explosion (64 cores × 1 thread = 64 threads)
...
def worker_init():
    """Initialize worker process with CPU-only mode and strict thread limits"""
    ...
    # Limit TensorFlow threading explicitly
    tf.config.threading.set_inter_op_parallelism_threads(1)
    tf.config.threading.set_intra_op_parallelism_threads(1)
...
if __name__ == '__main__':
    ...
    print(f"  Parallel workers: {CONFIG['num_workers']} CPU cores (1 thread each)")
    print()

#!/usr/bin/env python3
"""
Download Dailymotion videos using yt-dlp
"""
...
class DailymotionDownloader:
    def __init__(self, input_json, output_dir="downloaded_dailymotion", max_workers=8):
    ...
    def download_single_video(self, video_info):
        """Download a single Dailymotion video"""
        url = video_info['url']
        ...
        # Output template
        output_template = str(self.output_dir / f"dailymotion_{video_id}_{safe_title}.%(ext)s")

        # yt-dlp command for Dailymotion
        cmd = [
            'yt-dlp',
            '--no-warnings',
            '--format', 'best',
    ...
    def download_all(self):
        ...
        print("╔════════════════════════════════════════════════════════════════════════════╗")
        print("║                    DAILYMOTION VIDEO DOWNLOADER                            ║")
        print("╚════════════════════════════════════════════════════════════════════════════╝")
        print()
...
def main():
    ...
    # Configuration
    INPUT_JSON = sys.argv[1] if len(sys.argv) > 1 else "dailymotion_fight_videos.json"
    OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else "downloaded_dailymotion"
    ...
    if not Path(INPUT_JSON).exists():
        ...
        print()
        print("Example:")
        print(f"  python3 {sys.argv[0]} dailymotion_fight_videos.json downloaded_dailymotion 8")
        sys.exit(1)

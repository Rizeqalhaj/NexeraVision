#!/usr/bin/env python3
"""
Download WorldStar videos using yt-dlp
"""
...
class WorldStarDownloader:
    def __init__(self, input_json, output_dir="downloaded_worldstar", max_workers=8):
    ...
    def download_single_video(self, video_info):
        """Download a single WorldStar video"""
        url = video_info['url']
        ...
        # Extract ID from URL
        video_id = url.split('/')[-1] if url.split('/')[-1] else 'unknown'

        # Output template
        output_template = str(self.output_dir / f"worldstar_{video_id}_{safe_title}.%(ext)s")
    ...
    def download_all(self):
        ...
        print("╔════════════════════════════════════════════════════════════════════════════╗")
        print("║                    WORLDSTAR VIDEO DOWNLOADER                              ║")
        print("╚════════════════════════════════════════════════════════════════════════════╝")
        print()
...
def main():
    ...
    # Configuration
    INPUT_JSON = sys.argv[1] if len(sys.argv) > 1 else "worldstar_fight_videos.json"
    OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else "downloaded_worldstar"
    ...
    if not Path(INPUT_JSON).exists():
        ...
        print()
        print("First run: python3 scrape_worldstar.py")
        print("Then run: python3 download_worldstar_videos.py")
        sys.exit(1)

    # Run downloader
    downloader = WorldStarDownloader(

...
Optimized for TikTok's video format and URL structure
"""
...
class TikTokDownloader:
    def __init__(self, input_json, output_dir="downloaded_tiktok", max_workers=6):
    ...
    def download_single_video(self, video_info):
        ...
        # Sanitize filename
        safe_desc = "".join(c for c in description if c.isalnum() or c in (' ', '-', '_')).strip()[:50]

        # Output template
        output_template = str(self.output_dir / f"tiktok_{hashtag}_{video_id}_{safe_desc}.%(ext)s")

        # yt-dlp command for TikTok
        cmd = [
            'yt-dlp',
            '--no-warnings',
            '--format', 'best',  # TikTok videos come in specific format
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {'status': 'success', 'url': url, 'description': description}
            else:
    ...
    def download_all(self):
        ...
        print()
        print(f"⚠️  Note: TikTok downloads may be slower due to rate limiting")
        print()
        ...
        # Download in parallel (fewer workers for TikTok to avoid rate limits)
        success_count = 0
        failed_count = 0
        skipped_count = 0
        ...
        if failed_count > 0:
            ...
            print(f"   Common failures: Region-locked, removed, or age-restricted videos")
...
def main():
    ...
    # Configuration
    INPUT_JSON = sys.argv[1] if len(sys.argv) > 1 else "tiktok_cctv_fights.json"
    OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else "downloaded_tiktok"
    MAX_WORKERS = int(sys.argv[3]) if len(sys.argv) > 3 else 6  # Fewer workers for TikTok
    ...
    if not Path(INPUT_JSON).exists():
        ...
        print()
        print("Example:")
        print(f"  python3 {sys.argv[0]} tiktok_cctv_fights.json downloaded_tiktok 6")
        sys.exit(1)

...
1x A100 SXM4 (80GB VRAM) - Batch size 48 for maximum speed
"""
...
# Enable A100 with memory growth
gpus = tf.config.list_physical_devices('GPU')
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        print(f"✅ Enabled {len(gpus)} GPU(s) - A100 ready!")
    except RuntimeError as e:
        print(f"⚠️  GPU setup error: {e}")
else:
    print("❌ NO GPUs DETECTED!")
    print("Run: python3 /workspace/CHECK_GPU.py")
    exit(1)

class VideoDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, video_paths, labels, preprocessor, batch_size=48, shuffle=True, augment=False):
...
def main():
    ...
    # A100 can handle LARGE batches - 80GB VRAM!
    BATCH_SIZE = 48
    ...
    if ckpt['phase'] == 'finetuning':
        ...
        print(f"   Expected time: ~{(total_epochs - last_epoch) * 0.3:.1f} minutes")
        ...
        if last_epoch >= initial_epochs:
            ...
            print(f"   Expected time: ~{finetune_epochs * 0.3:.1f} minutes")
            ...
            print(f"\n🚀 Starting initial training from epoch {last_epoch}/{initial_epochs}")
            print(f"   Expected time: ~{(initial_epochs - last_epoch) * 0.3:.1f} minutes")
